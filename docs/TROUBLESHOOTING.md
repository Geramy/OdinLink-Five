...
| Module loaded, no `/dev/odl_tb5_*`, no `probed device` line | The other machine is not advertising OdinLink. Load `odl_tb5` on **both** sides. A live Thunderbolt host (`thunderbolt 0-1: new host found`) is not enough — `thunderbolt-net` / ThunderboltIP (`en05`, "login timed out") is a different protocol. After 15 s the driver prints `still no peer advertising protocol`. Use `loopback=1` for no-cable testing |
