...
static int nhi_path_alloc_hopid(struct odl_tb5_device *dev, int remote_tx_hopid)
{
	return tb_xdomain_alloc_in_hopid(nhi_priv(dev)->xd, remote_tx_hopid);
}

static void nhi_path_release_hopid(struct odl_tb5_device *dev, int in_hopid)
{
	tb_xdomain_release_in_hopid(nhi_priv(dev)->xd, in_hopid);
}
...
static void nhi_path_disable(struct odl_tb5_device *dev, int in_hopid)
{
	struct odl_tb5_transport_ring_info tx_info = nhi_tx_ring_info(dev);
	struct odl_tb5_transport_ring_info rx_info = nhi_rx_ring_info(dev);

	tb_xdomain_disable_paths(nhi_priv(dev)->xd,
				 nhi_priv(dev)->local_tx_hopid,
				 tx_info.hop,
				 in_hopid,
				 rx_info.hop);
...
const struct odl_tb5_transport_ops odl_tb5_nhi_transport = {
	...
	.path_alloc_hopid   = nhi_path_alloc_hopid,
	.path_release_hopid = nhi_path_release_hopid,
