...
struct odl_tb5_transport_ops {
	...
	/* Path setup is split so the caller can start and prime the RX ring
	 * between allocating the input hop-ID and enabling the path: frames
	 * arriving on a live path with no posted RX descriptors are dropped.
	 * Both teardown ops take the hop-ID explicitly because a re-login may
	 * already have overwritten dev->remote_tx_hopid. */
	int  (*path_alloc_hopid)(struct odl_tb5_device *dev, int remote_tx_hopid);
	void (*path_release_hopid)(struct odl_tb5_device *dev, int in_hopid);
	int  (*path_enable)(struct odl_tb5_device *dev);
	void (*path_disable)(struct odl_tb5_device *dev, int in_hopid);
